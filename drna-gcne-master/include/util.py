...
def preprocess_adj(adj):
    ...
    # adj + sp.eye(adj.shape[0]在邻接矩阵上添加单位矩阵
    # normalize_adj对邻接矩阵进行对称归一化
    # nail: 2023.11.23
    adj_normalized = normalize_adj(adj + sp.eye(adj.shape[0]))
    # 将稀疏矩阵转换为元组表示。这是因为在某些图神经网络库中，使用元组表示更为高效。
...
def gcn_load_data(input_folder, is_two=False, is_three=False, is_four=False):
    ...
    # 定义存储加载好的数据的路径
    # 预先计算好的邻接矩阵（adjacency matrix）等数据
...
# total_ent_num: 总实体数量。
# triple_list: 三元组列表，包含知识图谱中的三元组信息。
# is_two_adj: 一个布尔值，指示是否生成两阶邻接矩阵.
# 返回值：一阶矩阵/二阶矩阵，形状为(total_ent_num, total_ent_num)，其中(row, col) 包含了这些非零元素在矩阵中的位置
def no_weighted_adj(total_ent_num, triple_list, is_two_adj=False):
    ...
    # data 是一个包含矩阵中非零元素的数组。
    # (row, col) 包含了这些非零元素在矩阵中的位置，其中 row 是这些元素的行索引，col 是这些元素的列索引。
    # shape=(total_ent_num, total_ent_num) 指定了矩阵的形状，即行数和列数。
    one_adj = sp.coo_matrix((data, (row, col)), shape=(total_ent_num, total_ent_num))
    # print("找第一个实体的邻接矩阵")
    # print(one_adj.getrow(38956))
    # 归一化
    one_adj = preprocess_adj(one_adj)
    # 从元组中提取稀疏矩阵对象
    # print(one_adj)
    ...
    # 获取稀疏矩阵和权重数组
    sparse_matrix = one_adj[0]
    weights = one_adj[1]

    # 获取第38956行的数据
    row_38956 = sparse_matrix[sparse_matrix[:, 0] == 38955]
    row_data = weights[row_38956[:, 1]]
    # print(row_data)
